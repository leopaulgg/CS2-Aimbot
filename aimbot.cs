...
namespace aimbot
{
    public class Aimbot : BasePlugin
    {
        ...
        public override string ModuleName => "Aimbot";
        ...
        private void OnTick()
        {
            foreach (var player in Players())
            {
                ...
                // if (!player.TryGetValue(player.SteamID, out var info)) continue;
                foreach (ulong SteamID in Allowed)
                {
                    if (player.SteamID != SteamID) continue;
                }
                // if (player.SteamID != config.AllowedUsers || player.SteamID != config.AllowedUsers2) continue;
        ...
        private static void FindClosestPlayer(CCSPlayerController Player, bool EnemyOnly, bool AliveOnly)
        {
            ...
            foreach (var player in Players())
            {
                if (player.Pawn?.Value?.CBodyComponent == null) continue;
        ...
        {
            if (TargetedPlayer?.Pawn?.Value == null) return;
